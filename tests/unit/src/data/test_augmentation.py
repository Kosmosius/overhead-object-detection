...
def test_apply_augmentation_transformations(
    ...
    with patch('albumentations.Compose') as mock_compose:
        ...
        # Verify image dimensions remain the same
        assert result['image'].shape == sample_image.shape, "Augmented image dimensions should match the original."
...
def test_apply_augmentation_reproducibility(sample_image, sample_bboxes, sample_category_ids):
    ...
    with patch('albumentations.Compose') as mock_compose1, \
        ...
        assert result1 == result2, "Augmented results should be identical with the same seed."
...
def test_apply_augmentation_no_transform(sample_image, sample_bboxes, sample_category_ids):
    ...
    with patch('albumentations.Compose') as mock_compose:
        ...
        # Assertions
        assert result['image'] is sample_image, "Image should remain unchanged when no transforms are applied."
...
def test_apply_augmentation_empty_bboxes(sample_image):
    ...
    with patch('albumentations.Compose') as mock_compose:
        ...
        # Assertions
        assert result['image'] is sample_image, "Image should remain unchanged."
